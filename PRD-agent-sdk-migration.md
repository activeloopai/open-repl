...
The original plan was to expose all six OpenREPL tools as one in-process MCP server (`createSdkMcpServer`) for every agent. The end-to-end test (against a real Claude subscription) disproved that for **subagents**: an in-process SDK MCP server is only reachable from the **main thread**. A delegated subagent runs detached — its MCP tool calls come back `"Stream closed"`, and even built-in writes land non-deterministically across runs. Read-only subagents that just return text are reliable. So the **implemented contract** (see `agent/claude/roles.ts` + `roles.test.ts`) is:

- **Orchestrator (main thread, the lead engineer): the six MCP tools + `Agent`.** It does the file work itself through the in-process MCP tools (`read_file`, `write_file`, `list_dir`, `search_repo`, `run_command`, `run_app`) — reliable on the main thread, routed through `workspace.writeFile` + `probeApp`, firing the live-editor watcher. It drives the run → fix loop and applies reviewer findings before finishing.
- **Planner / Reviewer: read-only built-in tools** — `Read`, `Glob`, `Grep` only (no `Write`/`Edit`/`Bash`; their output is text, so async delegation can't corrupt file state). There is no separate `coder` subagent — coding lives on the orchestrator.
- **Command allowlist** (`config.commandAllowlist`) is enforced in `canUseTool` against the `run_command` MCP tool via the shared `isCommandAllowed` predicate, which rejects shell-operator chaining and matches on a command boundary. A read-only **loop guard** in `canUseTool` denies a 3rd identical read-only call to break stuck loops.

Set `settingSources: []`, `strictMcpConfig: true`, and `cwd: workspace.root`; `allowedTools` = `Agent` + read-only built-ins (`Read`/`Glob`/`Grep`) + the six `mcp__openrepl__*` tools.
